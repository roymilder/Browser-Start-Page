...
const tiles = [
  ...
  [
    ...
    { name: 'google-contacts', title: 'Google Contacts', href: 'href="https://contacts.google.com/' },
...
window.onload = () => {
  elements.slots = document.getElementById('slots');
  elements.tileList = document.getElementById('tileList');
  ...
  // Allow slots to be selected by clicking the icon or pressing the corresponding number key
  elements.slots.addEventListener('click', selectSlotOnClick);
  window.addEventListener('keypress', selectSlotOnKeyPress);
};
...
function selectSlot(slot) {
  if (tiles.length - 1 < slot) {
    return;
  }

  elements.slots.className = `slots slot-${slot}`;
  elements.tileList.innerHTML = '';

  for (let tile of tiles[slot]) {
    let listItem = document.createElement('li');
    listItem.className = `tile ${tile.name}`;

    let anchor = document.createElement('a');
    anchor.href = tile.href;
    anchor.title = tile.title;

    if (tile.href && tile.name) {
      let image = document.createElement('img');
      image.className = 'tile';
      image.src = `img/tiles/${tile.name}.png`;
       anchor.appendChild(image);
    }

    listItem.appendChild(anchor);
    elements.tileList.appendChild(listItem);
  }
}


/**
 * Whenever a slot icon is clicked, selects the corresponding slot
 */
function selectSlotOnClick(event) {
  if (event.srcElement) {
    let slot = event.srcElement.getAttribute('data-slot');
...
/**
 * Whenever a number key is pressed, selects the corresponding slot
 */
function selectSlotOnKeyPress(event) {
  if (/\d/.test(event.key)) {
    // It's a number key, so select the corresponding slot
